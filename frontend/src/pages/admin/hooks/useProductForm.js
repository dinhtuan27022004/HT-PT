    ...
    const onFinish = async (v) => {
        try {
            ...
            const payload = {
                ...
                specs: attributes.filter(a => a.type === 'text' && attributeValues[a.name]).reduce((s, a) => ({ ...s, [a.name]: attributeValues[a.name] }), {}),
                variants: variants.map(vr => ({ id: (vr.key && !vr.key.toString().startsWith('new-')) ? vr.key : null, name: vr.name, sku: vr.sku, price: vr.price, stock: vr.stock, status: vr.status, options: vr.options, compare_at: null, cost: null })),
                attribute_images: [
