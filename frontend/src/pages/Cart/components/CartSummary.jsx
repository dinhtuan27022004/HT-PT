...
const CartSummary = ({ total, count, formatPrice, onCheckout, disabled }) => (
    <Card title={<Title level={4} style={{ margin: 0 }}>Tóm tắt đơn hàng</Title>} style={{ borderRadius: '12px' }}>
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Text type="secondary">Tạm tính ({count} sản phẩm đã chọn):</Text>
            ...
            <Button
                ...
                size="large"
                block
                style={{ height: '50px', borderRadius: '8px', fontSize: '18px', fontWeight: 'bold' }}
                onClick={onCheckout}
                disabled={disabled}
            >
                MUA HÀNG ({count})
